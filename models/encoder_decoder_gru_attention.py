from tensorflow.python.keras.layers import Input,Embedding,GRU,Dense,TimeDistributed,Concatenate
...
def define_nmt(hidden_size,embedding_dim,source_lang_timesteps,source_lang_vocab_size,target_lang_timesteps,target_lang_vocab_size,dropout):
    ...
    inf_encoder_outputs=Input(shape=(source_lang_timesteps,hidden_size,))
    ...
    inf_attn_out, inf_attn_states = attn_layer([inf_encoder_outputs, inf_decoder_out])
    inf_decoder_concat = Concatenate(axis=-1, name='concat')([inf_decoder_out, inf_attn_out])

    inf_decoder_pred = TimeDistributed(dense)(inf_decoder_concat)
    decoder_model = Model(inputs=[inf_encoder_outputs,inf_decoder_inputs,inf_decoder_state],outputs=[inf_decoder_pred,inf_attn_states,inf_decoder_state_out])
...
def translate(sentence,encoder_model,decoder_model,source_tokenizer,target_tokenizer,src_vsize,tgt_vsize,source_timesteps,target_timesteps):
    ...
    while continuePrediction:
        decoder_pred,attn_state,decoder_state=decoder_model.predict([encoder_out,target_preproc_text,enc_last_state])
